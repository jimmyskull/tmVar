...
=item stem({ -words => \@words, -locale => 'no', -exceptions => \%exceptions });
...
Example:
  ...
  my $stemmed_words = Lingua::Stem::No::stem({ -words => \@words,
                                              -locale => 'no',
...
sub stem {
    ...
    my $stemmer   = Lingua::Stem::Snowball::No->new( use_cache => $use_cache );
    foreach (@$words) {
